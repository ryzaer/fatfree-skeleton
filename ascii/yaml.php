 ...
 * <code>
 *   $yaml  = new yaml();
 *   $array = $yaml->dump($array);
 *   $array = $yaml->load($file|$string);
 *   or;
 *   yaml::emit($array);
 *   yaml::parse($file|$string);
 * </code>
 *
...
class yaml
{
    ...
    static function emit($args){
        self::$stat = self::$stat ? self::$stat : new yaml();
        return self::$stat->dump($args);
    }

    static function parse($args){
        self::$stat = self::$stat ? self::$stat : new yaml();
        return self::$stat->load($args);
    }
    ...
    {
        $string =  file_exists($file) ? $this->loadFromFile($file) : $this->loadFromString($file) ;
        return $this->loadWithSource($string);
    }
    ...
    private function loadWithSource($Source)
    {
        ...
        if (($key = array_search('...', $Source)) !== false) {
            // remove ...
            unset($Source[$key]);
        }
